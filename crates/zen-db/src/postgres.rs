...
impl DbConnection for PostgresConnection {
    ...
    async fn explain_query(
        ...
        // Maximum-information option set on both paths. Unavailable
        // options would error, so we only include ones present in
        // every supported server version (PG 13+ — already EOL'd
        // boundary):
        //
        //   ANALYZE   — execute + per-node actuals (analyze path only).
        //   BUFFERS   — shared/local hit/read/dirtied/written pages.
        //               Available outside ANALYZE since PG 13.
        //   TIMING    — per-node wall-clock (analyze path only — the
        //               option *exists* without ANALYZE but reports 0).
        //   VERBOSE   — output columns, schema-qualified names, the
        //               whole "Output: …" detail row.
        //   SETTINGS  — non-default planner settings (PG 12+). Tiny
        //               but invaluable when comparing plans across
        //               sessions: a stray `SET enable_hashjoin = off`
        //               jumps out of the JSON instead of vanishing
        //               silently.
        //   WAL       — WAL records / FPIs / bytes generated by the
        //               run (PG 13+). Zero on SELECT, populated for
        //               DML — useful for INSERT/UPDATE/DELETE perf
        //               work. Analyze path only (no execution = no WAL).
        //   FORMAT JSON — non-negotiable; the visualizer parses JSON.
        //
        // Things we intentionally skip:
        //   COSTS=on   — default; no need to set explicitly.
        //   SUMMARY    — default ON under ANALYZE; explicit toggle
        //                only matters in non-ANALYZE mode where it
        //                buys us nothing.
        //   GENERIC_PLAN (PG 16+) — incompatible with ANALYZE and
        //                only meaningful for parameterised queries;
        //                separate feature if anyone wants it.
        //   MEMORY (PG 17+) — too new to assume.
        let opts = if analyze {
            "FORMAT JSON, ANALYZE, BUFFERS, VERBOSE, TIMING, SETTINGS, WAL"
        } else {
            "FORMAT JSON, VERBOSE, SETTINGS"
        };
