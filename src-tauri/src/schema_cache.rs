...
//!
//! Sibling of the user-config store; see **`docs/STORAGE.md`** for how
//! the two databases divide responsibility and why they live in
//! separate files. This module focuses on the autocomplete cache.
//!
...
//! [`zen_db::TableDescription`] payloads here and only refresh them when
//! either:
//!
...
//!
//! Storage shape (single file at `app_data_dir()/schema_cache.db`):
//!
//! ```sql
...
/// Thread-safe handle around the open SQLite connection. Held inside a
/// `parking_lot::Mutex` because rusqlite's `Connection` is `!Sync`; the
/// schema-cache operations are short and entirely CPU/disk-bound, so a
/// blocking mutex is appropriate.
#[derive(Clone)]
...
impl SchemaCache {
    /// Open (or create) the cache at the canonical location for the
    /// current app. Creating the parent directory and the schema is
    /// idempotent.
    ...
    pub fn open_at(path: &str) -> AppResult<Self> {
        let conn = Connection::open(path)
            .map_err(|e| AppError::Other(format!("schema_cache open: {e}")))?;
        // Reasonable durability vs. throughput trade-off for a cache.
        conn.execute_batch(
        ...
        .map_err(|e| AppError::Other(format!("schema_cache init: {e}")))?;
        Ok(Self {
    ...
    pub fn get(
        ...
        let row = conn
            ...
            .optional()
            .map_err(|e| AppError::Other(format!("schema_cache get: {e}")))?;
        match row {
            None => Ok(None),
    ...
    pub fn get_many(
        ...
        let mut stmt = conn
            .prepare(&sql)
            .map_err(|e| AppError::Other(format!("schema_cache get_many prepare: {e}")))?;
        ...
        let rows = stmt
            ...
            .map_err(|e| AppError::Other(format!("schema_cache get_many query: {e}")))?;

        let mut out = Vec::new();
        for row in rows {
            let (payload, indexed_at) =
                row.map_err(|e| AppError::Other(format!("schema_cache get_many row: {e}")))?;
            let description: TableDescription = serde_json::from_str(&payload)?;
    ...
    pub fn upsert(
        ...
        .map_err(|e| AppError::Other(format!("schema_cache upsert: {e}")))?;
        Ok(())
    }
    ...
    pub fn invalidate(
        ...
        if tables.is_empty() {
            ...
            .map_err(|e| AppError::Other(format!("schema_cache invalidate-all: {e}")))?;
            return Ok(());
        }
        let placeholders: Vec<String> = (0..tables.len()).map(|i| format!("?{}", i + 4)).collect();
        let sql = format!(
        ...
        conn.execute(&sql, params_from_iter(binds.iter().copied()))
            .map_err(|e| AppError::Other(format!("schema_cache invalidate: {e}")))?;
        Ok(())
    }
    ...
    pub fn invalidate_connection(&self, connection_id: &str) -> AppResult<()> {
        let conn = self.inner.lock();
        conn.execute(
            "DELETE FROM table_schema WHERE connection_id = ?1",
            params![connection_id],
        )
        .map_err(|e| AppError::Other(format!("schema_cache invalidate_connection: {e}")))?;
        Ok(())
    }
    ...
    pub fn list_cached(
        ...
        let mut stmt = conn
            ...
            .map_err(|e| AppError::Other(format!("schema_cache list prepare: {e}")))?;
        let rows = stmt
            ...
            .map_err(|e| AppError::Other(format!("schema_cache list query: {e}")))?;
        let mut out = Vec::new();
        for row in rows {
            out.push(row.map_err(|e| AppError::Other(format!("schema_cache list row: {e}")))?);
        }
        Ok(out)
    }
}
...
fn cache_path(app: &AppHandle) -> AppResult<String> {
    let dir: PathBuf = app
        .path()
        .app_data_dir()
        .map_err(|e| AppError::Other(format!("resolve app_data_dir: {e}")))?;
    std::fs::create_dir_all(&dir)?;
    Ok(dir.join("schema_cache.db").to_string_lossy().into_owned())
}
...
mod tests {
    use super::*;
    use zen_db::{ColumnDescription, TableKind};
