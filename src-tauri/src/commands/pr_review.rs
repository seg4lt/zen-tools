...
/// * The comment reads as if a human reviewer wrote it: a concise,
///   PR-ready comment followed by the deeper rationale, with an optional
///   GitHub-flavoured suggestion block when a concrete replacement exists.
///
...
mod tests {
    ...
    fn finding(severity: &str, suggested: &str) -> Finding {
        Finding {
            ...
            comment: "This invalid token stops the declaration block from parsing.".into(),
    ...
    fn empty_suggestion_omits_block() {
        ...
        assert!(body.contains("This invalid token stops"));
        assert!(body.contains("Details:"));
        assert!(body.contains("The token is not valid CSS"));
    }
